...
namespace Ventrix.Infrastructure.Repositories
{
    public class InventoryRepository : IInventoryRepository
    {
        ...
        {
            // FIX: Safely parse the string to the ItemStatus Enum before querying
            if (Enum.TryParse<ItemStatus>(status, true, out var statusEnum))
            {
                return await _context.InventoryItems.CountAsync(i => i.Status == statusEnum);
            }

            // Return 0 if someone accidentally passes an invalid status string
            return 0;
        }
